  ...
  <div class="left-navbar">
          ...
          <el-collapse-item v-for="(item, index) in tagLog" :key="index" :title="tagLogDate(item.time.$date)">
            <div>
              <div v-if="item.add.length">
                ...
                <el-tag v-for="tag in item.add" :key="tag" type="success" style="margin-right:5px;margin-top:3px">{{
                  tag
                }}</el-tag>
                <br />
              </div>
              ...
              <div v-if="item.del.length">
                ...
                <el-tag v-for="tag in item.del" :key="tag" type="danger" style="margin-right:5px;margin-top:3px">{{
                  tag
                }}</el-tag>
                <br />
              </div>
    ...
    <div class="left_list">
      ...
      <div class="titleTag">
        ...
        <p v-if="$route.path === '/video' && isLogin == true" @click="postVideo">{{ $t('tag.video_action') }}</p>
            ...
            <div class="tag belong-to-home">
              <div class="tag-div">
                ...
                <div v-if="colorTagList.indexOf(val) != -1">
                  <p :ref="val" :class="val" @click="gotoHome(key)">{{ key.replace(/_/g, ' ') }}</p>
                </div>
                <!-- 其他情况 -->
                <div v-else>
      ...
      <ul v-if="$route.path === '/home' && name === 'sub'" class="tag-ul">
        <li v-for="(val, key) in msg" :key="key" class="tag belong-to-home">
          <div class="tag-div">
            ...
            <!-- 存在标签颜色 -->
            <div v-if="colorTagList.indexOf(val) != -1">
              <p :class="val" @click="gotoHome(val)">{{ val }}</p>
            </div>
            <!-- 其他情况 -->
            <div v-else>
            ...
            <div v-for="item in key" :key="item" :class="val != 'Author' && 'tag'">
              ...
              >
                ...
                <div v-if="colorTagList.indexOf(val) != -1 && val != 'Author'">
                  ...
                  <el-button v-if="val == 'Author'" size="mini" style="margin-left:5px;" @click="openAuthorData(item)"
                    >详情</el-button
                  >
                </div>
...
<script>
import ShowAuthorData from '../components/page/home/ShowAuthorData.vue'
import EditTags from '~/components/page/home/EditTags.vue'
      ...
      this.colorTagList.forEach((i) => {
        if (this.$refs[i]) {
          this.$refs[i].forEach((i2) => {
            if (
              Math.floor(this.getTextSize(i2.textContent, '100%', '"Avenir", Helvetica, Arial, sans-serif').width) >
              i2.offsetWidth
            ) {
